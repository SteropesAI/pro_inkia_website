...
const SUPPORT_PROMPT_NOTE: Record<SupportId, string> = {
  toile: "printed on gallery canvas",
  acrylique: "printed on acrylic glass panel",
  aluminium: "printed on brushed aluminum panel",
  affiche: "fine art poster print on paper",
  bois: "printed on natural wood panel",
};
...
export function buildPrompt(selection: WizardSelection): string {
  ...
  const couleurIds = selection.couleur;
  const mergedPalettes = couleurIds.flatMap((id) => COLOR_PALETTE_POOLS[id]);
  const colors = pick(mergedPalettes.length ? mergedPalettes : COLOR_PALETTE_POOLS[couleurIds[0]]);
  ...
  const base = templateFor(selection.styleFamily, {
    element,
    colors,
    adjective,
    aspect,
  ...
  const supportNote = SUPPORT_PROMPT_NOTE[selection.support];
  if (base.includes(" --")) {
    return base.replace(" --", `, ${supportNote} --`);
  }
  return `${base}, ${supportNote}`;
}
...
export function buildSummaryLabels(selection: WizardSelection): SummaryLabels {
  return {
    ...
    couleur: labelsForCouleurs(selection.couleur),
    taille: labelFor("taille", selection.taille),
    cm: cmForTaille(selection.taille),
    support: labelFor("support", selection.support),
  };
}
...
export function isValidSelection(body: unknown): body is WizardSelection {
  ...
  const ids = {
    ...
    support: ["toile", "acrylique", "aluminium", "affiche", "bois"],
  } as const;
  for (const key of Object.keys(ids) as (keyof typeof ids)[]) {
    if (key === "couleur") {
      const arr = b.couleur;
      if (!Array.isArray(arr) || arr.length === 0) return false;
      if (!arr.every((c) => typeof c === "string" && (ids.couleur as readonly string[]).includes(c))) {
        return false;
      }
      continue;
    }
